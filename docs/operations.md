...
5. **UI** (`fleuve-ui`) — optional; serves the admin JSON API from **PostgreSQL** using the same `database_url` / `FLEUVE_DATABASE_URL` as other binaries. Set `FLEUVE_FRONTEND_DIST` (or `-frontend`) to serve static assets.
